...
class Program
{
    static void Main(string[] args)
    {
        ...
        using (RSA rsa = RSA.Create(2048))
        {
            ...
            Console.WriteLine($"PFX bytes length: {pfxBytes.Length}");
            File.WriteAllBytes("localhost.pfx", pfxBytes);
            Console.WriteLine("PFX file written");

            // Export as CER for trust store
            var cerBytes = certificate.Export(X509ContentType.Cert);
            Console.WriteLine($"CER bytes length: {cerBytes.Length}");
            File.WriteAllBytes("localhost.cer", cerBytes);
            Console.WriteLine("CER file written");
